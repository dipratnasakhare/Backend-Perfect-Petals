...
AdminSideRoutes.post("/OrderPost", async (req, res) => {
  ...
  try {
    ...
    if(User_Details.length == 0){
      ...
      let save = new ModelOrderDetails(req.body);
      await save.save()
    }else{
      User_Details[0].TotalPrice += req.body.TotalPrice
      User_Details[0].OrderDetails = [...User_Details[0].OrderDetails, ...req.body.OrderDetails]
     let x = await ModelOrderDetails.updateOne({ UserId }, User_Details[0]);
    }
...
AdminSideRoutes.post("/get-my-account", async (req, res) => {
  let { UserId } = req.body

  try {
    let data =  await ModelUserAuth.find({ UserId });
    data = data[0]
    data.password = "*********"
      res
      .status(200)
